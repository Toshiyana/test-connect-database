...
func main() {
	...
	conn, err := sql.Open("pgx", "host=localhost port=5432 dbname=test_connect user=toshi password=")
	if err != nil {
	...
	// insert a row
	// you should use "$", not inserting values directly.
...
func getAllRows(conn *sql.DB) error {
	...
	defer rows.Close() // Don't forget Close()
	...
	// After scanning every rows, you check for an error one more time(Best practice).
